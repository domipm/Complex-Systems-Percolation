...
int main(void) {
    ...
    FILE *file;
    char filename[100];
    sprintf(filename, "tri_%i.txt", L);
    file = fopen(filename, "w");
    fprintf(file, "probability\tavg cl size\n");
    ...
    Cluster *cluster;
    cluster = new Cluster[N_MAX];

    // Initialize random number generator
    extern gsl_rng *mu;
    mu = gsl_rng_alloc(gsl_rng_taus);
    gsl_rng_set(mu, time(NULL));

    // Generate empty lattice
    lattice.generate("tri", 0);

    // Expected position of peak
    double p_crit = 0.50;
    // Step for loop
    double step = 0.1;
    ...
    for (double k = 0.01; k < p_crit; k = k + k * step) {

        double p = p_crit - k;

        std::cout << "Iteration probability p = " << p << std::endl;
        ...
        // Activate or not nodes in lattice with probability p
        for (int n = 0; n < n_count; n++)
            if (gsl_rng_uniform(mu) < p)
                lattice.nodes[n].is_active = true;

        // *** HOSHEN-KOPELMAN ***

        // Set all nodes to those from lattice generate function (and restart needed properties)
        for (int i = 0; i < n_count; i++) {
            ...
            nodes[i].neighbors = std::vector<int>(0);
            nodes[i].N = n_count;
        ...
        // Write to file probability and average cluster size
        fprintf(file, "%f\t%f\n", p, avg_csize);
    ...
    // LOOP OVER PROBABILITIES
    for (double k = 0.01; k < 1 - (p_crit - 0.02); k = k + k * step) {

        double p = (p_crit - 0.02) + k;
