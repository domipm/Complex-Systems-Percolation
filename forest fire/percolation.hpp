...
class Node {
    ...
    public:
        ...
        // Each node has now a possibly different neighbor threshold distance
        float D;
        ...
        void find_neighbors(Node nodes[N_MAX]) {
            ...
            for (int i = 0; i < N; i++) {
                ...
                if (dist <= std::max(D, nodes[i].D) && dist != 0 && nodes[i].is_active == true)
